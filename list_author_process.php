...
echo "<tr><td width=50>SERIAL</td><td>NAME</td><td width=60>ACTION1</td><td width=60>ACTION2</td></tr>";
while($row=mysql_fetch_array($result)){
$author_id= $row['author_id'];		
$author_name= $row['author_name'];

if 	($author_name[0]==$test || $author_name[0] == $test2){
 echo "<tr><td>$sn</td><td>$author_name</td><td><form  action=edit_author.php onSubmit=return(readySubmit2())><input type=hidden name=id value=$author_id><input type=submit value=Edit ></form></td><td><form  action=delete_author.php onSubmit=return(readySubmit())><input type=hidden name=id value=$author_id><input type=submit value=Delete ></form></td></tr>";
 $sn++;
}
